...
class WC_Catalog_Restrictions_Filters {
	...
	public function __construct() {
		...
		add_action( 'woocommerce_init', array( $this, 'bind_filters_late' ) );
	...
	public function bind_filters_late() {
		...
		if ( WC_Catalog_Visibility_Compatibility::is_wc_version_gt( '3.4' ) ) {
			add_action( 'woocommerce_before_single_variation', array( $this, 'on_before_add_to_cart_button' ), 0 );
			add_action( 'woocommerce_after_single_variation', array( $this, 'on_after_add_to_cart_button' ), 998 );
			add_action( 'woocommerce_before_add_to_cart_button', array( $this, 'on_before_add_to_cart_button' ) );
			add_action( 'woocommerce_after_add_to_cart_button', array( $this, 'on_after_add_to_cart_button' ) );
		} else {
