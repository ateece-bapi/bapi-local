...
if (!class_exists('WCCS_Ajax')) {
	...
	class WCCS_Ajax {
		...
		public function wccs_toggle_zone_pricing() {
			if ( ! isset( $_POST['zp_nonce'] ) && ! wp_verify_nonce( sanitize_text_field( $_POST['zp_nonce'] ), 'custom_nonce' ) ) {
				exit('Unauthorized Request!');
			}
		...
		public function wccs_add_zone_pricing() {
			...
			if ( isset( $post['zp_name'] ) && isset( $post['zp_countries'] ) && isset( $post['zp_currency'] ) && isset( $post['zp_rate'] ) && ! empty( trim( $post['zp_name'] ) ) && ! empty( $post['zp_countries'] ) && ! empty( $post['zp_currency'] ) && ! empty( $post['zp_rate'] ) ) {
				...
				if ( false != $wccs_zp_data ) {
					foreach ( $wccs_zp_data as $key => $zp_data ) {
						...
						$html .= '<tr>
							<td><span class="zp_name">' . $zp_data['zp_name'] . '</span>
								<div class="wccs_zp_action">
									<a href="#" class="wccs_zp_edit" data-id="' . $key . '">Edit</a> | <a href="#" class="wccs_zp_delete" data-id="' . $key . '">Delete</a>
								</div>
							</td>
							<td><span class="zp_countries">' . $zp_countries . '</span></td>
							<td><span class="zp_currency">' . $zp_data['zp_currency'] . '</span></td>
							<td><span class="zp_rate">' . wp_kses_post( number_format( $zp_data['zp_rate'], 2 ) ) . '</span></td>
						</tr>';
					}
				}				
		...
		public function wccs_update_all_callback() {
			...
			print_r(json_encode(array('status' => $status, 'rates' => $rates)));
