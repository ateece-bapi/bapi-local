...
if ( ! class_exists( 'WCCS' ) ) {
	...
	class WCCS {
		...
		public function __construct() {
			...
			// override price
			add_filter( 'woocommerce_product_get_price', array( $this, 'wccs_custom_price' ), 9, 2 );
			add_filter( 'woocommerce_product_get_sale_price', array( $this, 'wccs_custom_price' ), 9, 2 );
			add_filter( 'woocommerce_product_get_regular_price', array( $this, 'wccs_custom_price' ), 9, 2 );
			...
			// Variations
			add_filter( 'woocommerce_product_variation_get_regular_price', array( $this, 'wccs_custom_variable_price' ), 9, 2 );
			// add_filter('woocommerce_product_variation_get_sale_price', array($this, 'wccs_custom_variable_price'), 9, 2);
			add_filter( 'woocommerce_product_variation_get_price', array( $this, 'wccs_custom_variable_price' ), 9, 2 );
			...
			add_filter( 'woocommerce_variation_prices_price', array( $this, 'wccs_custom_variation_price' ), 99, 3 );
			add_filter( 'woocommerce_variation_prices_regular_price', array( $this, 'wccs_custom_variation_price' ), 99, 3 );
			add_filter( 'woocommerce_variation_prices_sale_price', array( $this, 'wccs_custom_variation_price' ), 99, 3 );
			...
			// add sticky switcher
			add_action( 'wp_footer', array( $this, 'wccs_add_sticky_callback' ) );
			...
			add_action( 'woocommerce_checkout_create_order', array( $this, 'wccs_change_order_currency' ), 10, 1 );
		...
		public function wccs_refresh_cart_subtotal( $fragments ) {
			...
			echo wp_kses_post( wc_price( $woocommerce->cart->total ) );
		...
		/*
		public function woocommerce_booking_product_price( $price, $Product, $posted_data ) {
		...
		public function wccs_register_order_meta_box() {
			...
			if ( ! empty( get_post_meta( $post->ID, '_wccs_shop_currency', true ) ) ) {
				add_meta_box( 'wccs-currency-metabox', esc_html__( 'WCCS Order Info', 'wccs' ), array( $this, 'wccs_render_order_metabox_html' ), 'shop_order', 'side', 'core' );
			}
		}

		public function wccs_render_order_metabox_html() {
			global $post;
			printf( esc_html( '%s %s' ) . esc_html__( 'Order Currency:', 'wccs' ) . esc_html( '%s ' . get_post_meta( $post->ID, '_wccs_shop_currency', true ) . '%s' ), '<p>', '<strong>', '</strong>', '</p>' );
			printf( esc_html( '%s %s' ) . esc_html__( 'Base Currency:', 'wccs' ) . esc_html( '%s ' . get_post_meta( $post->ID, '_wccs_base_currency', true ) . '%s' ), '<p>', '<strong>', '</strong>', '</p>' );
			printf( esc_html( '%s %s' ) . esc_html__( 'Order Currency rate:', 'wccs' ) . esc_html( '%s ' . get_post_meta( $post->ID, '_wccs_currency_rate', true ) . '%s' ), '<p>', '<strong>', '</strong>', '</p>' );
			printf( esc_html( '%s %s' ) . esc_html__( 'Total Amount:', 'wccs' ) . esc_html( '%s ' . get_post_meta( $post->ID, '_wccs_total_in_base_currency', true ) . '%s' ), '<p>', '<strong>', '</strong>', '</p>' );
		}
		...
		public function wccs_change_order_currency( $order ) {
			...
			if ( ! empty( $this->wccs_get_currency() ) ) {
				if ( ! get_option( 'wccs_pay_by_user_currency' ) && $this->wccs_get_currency() != $this->wccs_get_default_currency() ) {
					// echo 'pay in base currency ';
					// Get order total
					// $order_tax = $order->data['total_tax'];
					$coversion_rate = $this->wccs_get_currency_rate();
					$decimals       = $this->wccs_get_currency_decimals();
					// $new_total = round( ($order_total / $coversion_rate), $decimals);
					// $order->data['shipping_total'] = round( ($order->data['shipping_total'] / $coversion_rate), $decimals);
					...
					$order->set_currency( $this->wccs_get_default_currency() );

					foreach ( $order->get_items() as $item_id => $item ) {

						$product_price = $item->legacy_values['line_total']; // product Quantity
						// $product_quantity = (int) $item->get_quantity(); // product Quantity
						$product_new_price = round( ( $product_price / $coversion_rate ), $decimals );

						// Set the new price
						$item->set_subtotal( $product_new_price );
						$item->set_total( $product_new_price );
		...
		public function wccs_admin_enqueue_assets( $hook ) {
			...
			if ( 'woocommerce_page_wccs-settings' == $hook || 'shop_coupon' == get_post_type() ) {
				...
				wp_enqueue_script( 'wccs_admin_settings_script', WCCS_PLUGIN_URL . 'assets/admin/js/setting_script.js', array( 'wccs_jquery_ui_script' ), '1.0&t=' . gmdate( 'H:i:s' ) );
				wp_localize_script(
		...
		public function wccs_price_format( $format, $currency_pos ) {
			...
			if ( ( ! is_admin() && ! empty( $current_currency ) ) || ( wp_doing_ajax() && ! empty( $current_currency ) ) ) {
				$current_pos    = $this->wccs_get_currency_format();
				$default_format = '%1$s%2$s';
				switch ( $current_pos ) {
					case 'left':
						$format = '%1$s%2$s';
						break;
		...
		public function wccs_detect_currency() {
			...
			if ( isset( $_REQUEST['nonce'] ) && ! wp_verify_nonce( sanitize_text_field( $_REQUEST['nonce'] ), 'custom_nonce' ) ) {
				exit( 'Not Authorized' );
			}
