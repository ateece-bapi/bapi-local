...
if ( ! class_exists( 'WCCS_Settings' ) ) {
	...
	class WCCS_Settings {
		...
		public function __construct() {
			...
			add_action('save_post', array( $this, 'wccs_zp_product_data_tab_content_save') );
		}

		public function wccs_zp_product_data_tab_content_save( $post_id ) {

			global $post;

			//die( 'post id is ' . $post_id );
		...
		public function wccs_zp_product_tab_content() {
			...
			<div id='wccs_zp_tab' class='panel woocommerce_options_panel hidden'>
				<div id="wccs_zp_pricing_view_for_product" >
					...
					<p class="notice-warning notice wccs_zp_notice"><?php echo sprintf( esc_html__( 'Entering a new value will override exchange rate in  %1$s Zone Pricing %2$s setting tab. Leave it empty for default setting.', 'wccs' ), '<em>', '</em>' ); ?></p>
					<?php 
					...
					if ( false != $wccs_zp_data && count( $wccs_zp_data ) > 0 ) {
						...
						foreach ( $wccs_zp_data as $key => $zp_data ) {
							...
							echo '<tr>								
								<td><span class="zp_name">' . esc_html( $zp_data['zp_name'] ) . '</span></td>
								<td><span class="zp_countries">' . esc_html( $zp_countries ) . '</span></td>
								<td><span class="zp_currency">' . esc_html( $zp_data['zp_currency'] ) . '</span></td>
								<td>
									<span class="zp_rate">
										<input type="hidden" name="zp_id[]" value="' . esc_attr( $key ) . '" />
										<input type="text" name="zp_rate[]" placeholder="' . wp_kses_post( number_format( $zp_data['zp_rate'], 2 ) ) . '" value="' . esc_attr( $individual_rate ) . '" style="margin-right: 10px" /> = 1 ' . esc_html( $this->default_currency ) . '
									</span>
								</td>
							</tr>';							
						}
		...
		public function wccs_custom_menu_page() {
			add_submenu_page( 'woocommerce', 'WC Currency Switcher Settings', 'WCCS Settings', 'administrator', 'wccs-settings', array( $this, 'wccs_settings_page_callback' ) );
			...
			<div class="wrap">
				...
				<form method="post" action="options.php">
						...
						<table class="form-table">
							<tr valign="top">
								...
								<td>
									...
									<p class="description"><?php echo sprintf( esc_html__( 'Enabling this option will override general settings except location IP feature. This option will require %1$s Shop Currency %2$s and %1$s Currency by Location %2$s to be enabled.', 'wccs' ), '<em>', '</em>' ); ?></p>
								</td>
							</tr>
						</table>
										...
										if ( false != $wccs_zp_data ) {
											foreach ( $wccs_zp_data as $key => $zp_data ) {
												...
												<tr>
													...
													<td>
														<span class="zp_countries"><?php print_r( esc_html( $zp_countries ) ); ?></span>
													</td>
													<td>
														<span class="zp_currency"><?php echo esc_html( $zp_data['zp_currency'] ); ?></span>
													</td>
													<td>
														<span class="zp_rate"><?php echo wp_kses_post( number_format( $zp_data['zp_rate'], 2 ) ); ?></span>
													</td>
												</tr>
												<?php
											}
					...
					if ( 'wccs-settings-general' == $active_tab ) {
						...
						//do_settings_sections( 'wccs-settings-general' );
						?>
										...
										foreach ( $flags as $country => $flag ) {
											foreach ( $currency_countries as $value ) {
												...
												if ( $country == $value ) {
													...
													$single_option .= '<option value="' . strtolower( $country ) . '" ' . @$selected . ' data-prefix="';
													$single_option .= "<span class='wcc-flag flag-icon flag-icon-" . strtolower( $country ) . "'></span>";
													$single_option .= '"';
													$single_option .= '> (' . $country . ')</option>';

													echo nl2br($single_option);
												}
											}
										}
										?>
